 ...
 */
package com.garytry.sqlmanager.core.resource.classpath;
...
import com.garytry.sqlmanager.core.resource.AbstractLoadableResource;
import com.garytry.sqlmanager.core.scanner.Location;
import com.garytry.sqlmanager.util.BomStrippingReader;
import com.garytry.sqlmanager.util.FileCopyUtils;
...
public class ClassPathResource extends AbstractLoadableResource implements Comparable<ClassPathResource> {
    ...
    public String getAbsolutePathOnDisk() {
        URL url = getUrl();
        if (url == null) {
            throw new sqlManagerException("Unable to fileNameWithAbsolutePath resource on disk: " + fileNameWithAbsolutePath);
        }
        try {
            return new File(URLDecoder.decode(url.getPath(), "UTF-8")).getAbsolutePath();
        } catch (UnsupportedEncodingException e) {
            throw new sqlManagerException("Unknown encoding: UTF-8", e);
        }
    }
    ...
    public LineReader loadAsString() {
        try {
            ...
            if (inputStream == null) {
                throw new sqlManagerException("Unable to obtain inputstream for resource: " + fileNameWithAbsolutePath);
            }
            return new DefaultLineReader(new BomStrippingReader(new InputStreamReader(inputStream, encoding)));
        } catch (IOException e) {
            throw new sqlManagerException("Unable to load resource: " + fileNameWithAbsolutePath + " (encoding: " + encoding + ")", e);
        }
    }
    ...
    public byte[] loadAsBytes() {
        try {
            ...
            throw new sqlManagerException("Unable to load resource: " + fileNameWithAbsolutePath, e);
        }
    }
